...
@section('title', __('account.balance.title') . ' · ' . $booking->booking_number)
    ...
    <div class="tg-breadcrumb-area pt-150 pb-90 p-relative" style="background: linear-gradient(rgba(0,0,0,.5),rgba(0,0,0,.5)), url('{{ asset('images/heroes/hero-bookings.jpg') }}') center/cover; background-color: var(--tg-theme-primary);">
        <div class="container">
            <div class="row"><div class="col-12 text-center text-white">
                <h1 class="mb-3 wow fadeInUp">{{ __('account.balance.title') }}</h1>
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb justify-content-center mb-0">
                        ...
                        <li class="breadcrumb-item active text-white" aria-current="page">{{ __('account.balance.balance') }}</li>
                    </ol>
                </nav>
            </div></div>
        </div>
        ...
        <div class="container">
            <div class="mx-auto" style="max-width:600px">
                <div class="text-center mb-4">
                    <h2 class="fw-bold mb-2" style="color:#0E1B33">{{ __('account.balance.complete_payment') }}</h2>
                    <p class="text-muted mb-0">{{ __('account.common.booking') }} <strong>#{{ $booking->booking_number }}</strong> · {{ $booking->tour->name ?? '' }}</p>
                </div>

                <div class="p-4 p-md-5 rounded-4" style="background:#f8fafc;border:1px solid #e9eef5">
                    <table class="w-100" style="font-size:.95rem">
                        <tr><td class="py-1 text-muted">{{ __('account.balance.booking_total') }}</td><td class="py-1 text-end">€ {{ number_format((float) $booking->total_amount, 2, ',', '.') }}</td></tr>
                        <tr><td class="py-1 text-muted">{{ __('account.balance.deposit_paid') }}</td><td class="py-1 text-end">− € {{ number_format((float) $booking->amount_paid, 2, ',', '.') }}</td></tr>
                        <tr style="border-top:1px solid #e9eef5"><td class="pt-2 fw-bold" style="color:#0E1B33">{{ __('account.balance.balance_due') }}</td><td class="pt-2 text-end fw-bold" style="color:var(--tg-theme-primary);font-size:1.3rem">€ {{ number_format($balanceAmount, 2, ',', '.') }}</td></tr>
                    </table>
                    ...
                    <small class="d-block text-muted text-center mt-3" style="font-size:.78rem">
                        <i class="fa-brands fa-cc-stripe me-1"></i>{{ __('account.balance.secure_stripe') }}
                    </small>
                </div>
            </div>
        </div>
