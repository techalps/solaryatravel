...
@section('title', __('account.transfer.transfer') . ' · ' . $booking->booking_number)
    ...
    <div class="tg-breadcrumb-area pt-150 pb-90 p-relative" style="background: linear-gradient(rgba(0,0,0,.5),rgba(0,0,0,.5)), url('{{ asset('images/heroes/hero-bookings.jpg') }}') center/cover; background-color: var(--tg-theme-primary);">
        <div class="container">
            <div class="row"><div class="col-12 text-center text-white">
                <h1 class="mb-3 wow fadeInUp">{{ __('account.transfer.registered') }}</h1>
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb justify-content-center mb-0">
                        ...
                        <li class="breadcrumb-item active text-white" aria-current="page">{{ __('account.transfer.transfer') }}</li>
                    </ol>
                </nav>
            </div></div>
        </div>
        ...
        <div class="container">
            <div class="mx-auto" style="max-width:680px">
                ...
                <div class="text-center mb-4">
                    ...
                    <h2 class="fw-bold mb-2" style="color:#0E1B33">{{ __('account.transfer.title') }}</h2>
                ...
                <div class="p-4 p-md-5 rounded-4" style="background:#f8fafc;border:1px solid #e9eef5">
                    <div class="text-center mb-4">
                        <div class="small text-uppercase fw-bold text-muted" style="letter-spacing:.06em">{{ __('account.transfer.amount') }}</div>
                        ...
                        @if($booking->payment_type === 'deposit')
                            <span class="badge text-bg-info">{{ __('account.transfer.deposit_then_balance') }}</span>
                        @endif
                    </div>

                    @if($bankDetails)
                        <div class="mb-3">
                            <div class="small text-uppercase fw-bold text-muted mb-1" style="letter-spacing:.06em">{{ __('account.transfer.bank_details') }}</div>
                ...
                <div class="text-center mt-4">
                    <a href="{{ route('booking.show', $booking->uuid) }}" class="tg-btn">{{ __('account.transfer.go_to_booking') }}</a>
                </div>
