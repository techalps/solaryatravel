    ...
    <div class="dash-page-header">
        <div>
            ...
            <p class="mb-0 text-muted">
                ...
                · {{ $departures->count() }} {{ \Illuminate\Support\Str::plural('partenza', $departures->count()) }}
            </p>
        </div>
    ...
    @if($departures->isEmpty())
        <div class="dash-card">
            <div class="dash-card-body text-center py-5">
                ...
                <p class="text-muted mb-0">Il {{ $date->format('d/m/Y') }} non ci sono partenze{{ $selectedTour ? ' per il tour selezionato' : '' }}. Usa le frecce per cambiare giorno.</p>
            </div>
        </div>
    @else
        <div class="row g-3">
                        ...
                        <div class="dash-card h-100 brd-card">
                            <div class="dash-card-body">
                                <div class="d-flex justify-content-between align-items-start mb-2 gap-2">
                                    <h3 class="h6 mb-0 fw-bold text-dark">{{ $dep->tour?->name ?? 'Tour' }}</h3>
                                    <span class="badge text-bg-primary flex-shrink-0">{{ $dep->confirmed_bookings_count }} prenot.</span>
                                </div>
                                <div class="text-muted small mb-3">
                                    <i class="bi bi-clock me-1"></i>{{ \Carbon\Carbon::parse($dep->start_time)->format('H:i') }}@if($dep->end_time) – {{ \Carbon\Carbon::parse($dep->end_time)->format('H:i') }}@endif
                                </div>
