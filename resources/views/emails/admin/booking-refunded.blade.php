...
<head>
<meta charset="UTF-8">
<title>Rimborso prenotazione</title>
</head>
    ...
    <tr>
        <td align="center">
            <table role="presentation" width="640" cellpadding="0" cellspacing="0" style="max-width:640px;background:#ffffff;border-radius:12px;overflow:hidden;box-shadow:0 4px 16px rgba(15,23,42,.06);">
                <tr>
                    <td style="background:linear-gradient(135deg,#f59e0b 0%,#d97706 100%);padding:32px 28px;color:#ffffff;">
                ...
                <tr>
                    <td style="padding:28px;">
                        <p style="margin:0 0 16px 0;font-size:15px;color:#475569;">
                            È stato registrato un rimborso per la prenotazione seguente.
                            @if($amount)<br>Importo rimborsato: <strong>€ {{ number_format((float) $amount, 2, ',', '.') }}</strong>@endif
                            @if($note)<br>Nota: {{ $note }}@endif
                        </p>
