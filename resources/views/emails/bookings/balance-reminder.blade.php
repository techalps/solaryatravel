...
<head>
<meta charset="UTF-8">
<title>Salda la tua prenotazione</title>
</head>
            ...
            <table role="presentation" width="640" cellpadding="0" cellspacing="0" style="max-width:640px;background:#ffffff;border-radius:12px;overflow:hidden;box-shadow:0 4px 16px rgba(15,23,42,.06);">
                <tr>
                    <td style="background:linear-gradient(135deg,#7c3aed 0%,#5b21b6 100%);padding:32px 28px;color:#ffffff;">
                        ...
                        <div style="font-size:24px;font-weight:700;margin-top:4px;">Completa il pagamento del saldo</div>
                    </td>
                </tr>
                <tr>
                    <td style="padding:28px;">
                        ...
                        <p style="margin:0 0 18px 0;line-height:1.6;">
                            si avvicina la data del tuo tour <strong>{{ $booking->tour->name ?? '' }}</strong>@if($booking->departure) del {{ \Carbon\Carbon::parse($booking->departure->departure_date)->format('d/m/Y') }}@endif.
                            Per completare la prenotazione <strong>#{{ $booking->booking_number }}</strong> ti ricordiamo di saldare l'importo residuo.
                        </p>

                        <div style="background:#f5f3ff;border:1px solid #ddd6fe;border-radius:8px;padding:16px;margin-bottom:18px;">
                            <table role="presentation" width="100%" cellpadding="0" cellspacing="0">
                                <tr><td style="padding:3px 0;font-size:14px;color:#475569;">Totale</td><td style="padding:3px 0;font-size:14px;text-align:right;">€ {{ number_format((float) $booking->total_amount, 2, ',', '.') }}</td></tr>
                                <tr><td style="padding:3px 0;font-size:14px;color:#475569;">Acconto versato</td><td style="padding:3px 0;font-size:14px;text-align:right;">− € {{ number_format((float) $booking->amount_paid, 2, ',', '.') }}</td></tr>
                                <tr><td style="padding:6px 0 0;font-size:15px;font-weight:700;color:#0f172a;">Saldo da pagare</td><td style="padding:6px 0 0;font-size:18px;font-weight:800;text-align:right;color:#5b21b6;">€ {{ number_format((float) $booking->balance_amount, 2, ',', '.') }}</td></tr>
                            </table>
                            @if($booking->balance_due_at)
                                <p style="margin:12px 0 0;font-size:13px;color:#92400e;">⏰ Salda entro il {{ \Carbon\Carbon::parse($booking->balance_due_at)->format('d/m/Y H:i') }}.</p>
                            @endif
                        </div>
